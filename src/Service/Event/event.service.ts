...
export class EventService {
  ...
  private hideOnContainerClick(graph: Graph): void {
    graph.getContainer().onmousedown = (): void => {
  ...
  private graphMouseEnterEvent(graph: Graph): void {
    graph.on("node:mouseenter", (e: IG6GraphEvent): void => {
      graph?.setItemState(e.item || "", "hover", true);
      let user: Vertex | undefined = this.userService.users.find(
        (item: Vertex): boolean => {
  ...
  private graphMouseClickEvent(graph: Graph): void {
    graph.on("node:click", (e: IG6GraphEvent): void => {
      if (this.getSelectedNodes(graph).length == 1) {
        let user: Vertex | undefined = this.userService.findById(
          parseInt(e.item?._cfg?.id || ""),
        );
